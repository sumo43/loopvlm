...
transformer_configs = {
    ...
    "paligemma-3b-mix-448": dict(block_size = 8192, dim=2048, vocab_size=257216, n_layer=18, n_head=8, n_local_heads=1, intermediate_size=16384,  rope_base=10000),

    "paligemma-3b-mix-224": dict(block_size = 8192, dim=2048, vocab_size=257216, n_layer=18, n_head=8, n_local_heads=1, intermediate_size=16384,  rope_base=10000),
...
class Transformer(nn.Module):
    def __init__(self, config: ModelArgs) -> None:
        ...
        self.tok_embeddings = nn.Embedding(config.vocab_size, config.dim, padding_idx=0)
    ...
    def setup_caches(self, max_batch_size, max_seq_length):
        ...
        self.freqs_cis = precompute_freqs_cis(8192 * 2, self.config.head_dim, self.config.rope_base)
        self.causal_mask = torch.tril(torch.ones(self.max_seq_length, self.max_seq_length, dtype=torch.bool))

        #self.causal_mask[:256, :256] = torch.ones_like(self.causal_mask[:256, :256]).to('cuda:0',
        #                                                                                        dtype=torch.bool)


    def forward(self, idx: Tensor, input_pos: Optional[Tensor] = None, embeds: Optional[Tensor] = None) -> Tensor:
